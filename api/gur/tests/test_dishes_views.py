...
class DishesViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        admin_user = CustomUser.objects.create_superuser(
            email='bla1@gmail.com', password='password', is_superuser=True
        )
        ...
        RestaurantAdmin.objects.create(rest_id=2, user_account=part_admin_account)
    ...
    def test_retrieve_exact_dish_non_admin(self):
        url = reverse('restaurant-dishes-exact', kwargs={
            'pk': 1,
            "restaurant_id": 1
        })
        response = self.client.get(url, {}, **self.header, format='json')
    ...
    def test_retrieve_exact_dish(self):
        ...
        response = self.client.get(url, {}, **self.admin_header, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.content)
        self.assertEqual(response.data['id'], 1, response.data)
    ...
    def test_update_existing_dish(self):
        ...
        prev_dish = Dish.objects.get(pk=1)
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.content)
        new_dish = Dish.objects.get(pk=1)
    ...
    def test_update_dish_from_other_restaurant(self):
        url = reverse('restaurant-dishes-exact', kwargs={'pk': 1, "restaurant_id": 1})
        response = self.client.patch(url, {
            "name": "Наливка \"Груша\"",
            "price": 3900,
            "gramme": 120
    ...
    def test_destroy_non_existing_dish(self):
        url = reverse('restaurant-dishes-exact', kwargs={'pk': 31, "restaurant_id": 1})
        response = self.client.delete(url, {}, **self.admin_header, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND, response.content)
        self.assertEqual(len(response.data), 1)
